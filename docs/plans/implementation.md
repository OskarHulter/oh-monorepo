...
The Ralph Loop runs **unsupervised**, so its allowlist is tighter than the [general agent allowlist](../operations/agent-permissions.md). Use the general one for human-in-the-loop sessions; use this one for autonomous runs.
