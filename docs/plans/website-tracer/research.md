...
2. **Decide and ADR**: `docs/adr/0003-tanstack-start-on-cloudflare-pages.md` capturing R1 outcome + plain-Vite-fallback boundary if applicable.
3. **Pick one deepening candidate** (1, 2, or 3 above) before writing the PRD — it shapes how `packages/client` is reorganized.
4. **Then**: PRD (step 4 in the cycle) using accepted research + ADR.
